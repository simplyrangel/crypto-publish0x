...
# -----------------------------------------------------
# read data:
gov_period2 = pd.read_hdf("bin/2022-02-02-algorand-governance-period-2.hdf")
...
plt.scatter(
    ...
    color="blue",
    s=200,
    label="Gov. period 2 (eligible 2022-02-02)",    
    )
...
# -----------------------------------------------------
gov_period2_total = gov_period2_eligible.committed_algos.sum()
cutoff_list = [1,2,3,4]+[5*x for x in range(1,60)]#range(5,300,5)
ratio_list = []
for ci in cutoff_list:
    dfi = gov_period2_eligible.sort_values(
        by="committed_algos",
        ascending=False,
        ).iloc[:ci,:].copy(
        )
    ratio = dfi.committed_algos.sum() / gov_period2_total
